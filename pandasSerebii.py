...
def treatSerebiiData():
  ...
  statsData = pd.read_json('pokemons_stats_serebii.json')

  ## -- Fazendo merge do dano por tipo num unico dataFrame -- ##
  goodInfo = pd.merge(general, statsData, left_on="Nome", right_on="Nome")

  goodInfo = pd.merge(goodInfo, damageData, left_on="Nome", right_on="Nome")
